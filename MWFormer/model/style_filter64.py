# Style filter. Generate a 64-dimension vector to represent the weather type.
# First extract a Gram matrix, then apply an MLP to it.
import numpy as np
import torch
...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128],
                 num_heads=[1, 2], mlp_ratios=[4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
...
class StyleFilter_conv1(nn.Module):
    def __init__(self, inputsize):
        super(StyleFilter_conv1, self).__init__()
...
def gram_matrix(tensor):
    ...
    gram = torch.bmm(tensor, tensor.permute(0,2,1))  # (b, d, h*w) (b, h*w, d) --> (b, d, d)
    return gram

class StyleFilter_res1(nn.Module):
    def __init__(self, inputsize):
        super(StyleFilter_res1, self).__init__()
...
class StyleFilter_Top(nn.Module):
    def __init__(self):
        super(StyleFilter_Top, self).__init__()
        self.encoder = EncoderTransformer(patch_size=4, embed_dims=[64, 128], num_heads=[1, 2], mlp_ratios=[2, 2],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2], sr_ratios=[4, 2],
    ...
    def forward(self,x):
        ...
        gram_matrix1 = gram_matrix(enc_out[0])
        gram_matrix2 = gram_matrix(enc_out[1])
        b1,d1,_ = gram_matrix1.shape    #batch per GPU
        b2,d2,_ = gram_matrix2.shape
        #print('b1 = ', b1)
        index1 = (torch.triu(torch.ones(d1, d1)) == 1)
        gram_vec1 = gram_matrix1[:, index1].view(b1,-1)
        index2 = (torch.triu(torch.ones(d2, d2)) == 1)
        gram_vec2 = gram_matrix2[:, index2].view(b2,-1)
        filter1_out = self.style_filter1(gram_vec1)
        filter2_out = self.style_filter2(gram_vec2)
        out = self.out1_fc(torch.cat((filter1_out, filter2_out),1))  
