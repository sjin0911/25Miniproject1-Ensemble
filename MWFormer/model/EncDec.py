...
class FilmBlock(nn.Module):  #input shape: n, c, h, w
    ...
    def __init__(self, cin_x, cin_y, x_out_channels):
        ...
        self.in_project_y = nn.Linear(self.x_out_channels, self.x_out_channels, bias=True)     
...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, feature_chans=64,  embed_dims=[64, 128, 256, 512],
        ...
        self.patch_embed1 = OverlapPatchEmbed(img_size=img_size, patch_size=7, stride=4, in_chans=embed_dims[0],
        ...
        self.patch_block1 = nn.ModuleList([Block(
            dim=embed_dims[1], hyper_attn=True, num_heads=num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
        ...
        self.block2 = nn.ModuleList([Block(
            dim=embed_dims[1], hyper_attn=False, num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
        ...
        self.patch_block2 = nn.ModuleList([Block(
            dim=embed_dims[2], hyper_attn=False, num_heads=num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[1])
            for i in range(1)])
        ...
        self.block3 = nn.ModuleList([Block(
            dim=embed_dims[2], hyper_attn=False, num_heads=num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
        ...
        self.patch_block3 = nn.ModuleList([Block(
            dim=embed_dims[3], hyper_attn=False, num_heads=num_heads[1], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[2])
            for i in range(1)])
        ...
        self.block4 = nn.ModuleList([Block(
            dim=embed_dims[3], hyper_attn=False, num_heads=num_heads[3], mlp_ratio=mlp_ratios[3], qkv_bias=qkv_bias, qk_scale=qk_scale,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
    ...
    def forward_features(self, x, feature_vec):
        B,_,H,W = x.shape
        vec = feature_vec
        feature_vec = feature_vec.unsqueeze(2).unsqueeze(3)
        _,c,_,_ = feature_vec.shape
        feature_vec0 = feature_vec.expand_as(torch.ones(B,c,H,W))
        x = self.input_film(x, feature_vec0)
        ...
        # stage 2
        feature_vec1 = feature_vec.expand_as(torch.ones(B,c,H1,W1))
        x1 = self.film1(x1, feature_vec1)
        ...
        # stage 3
        feature_vec2 = feature_vec.expand_as(torch.ones(B,c,H1,W1))
        x1 = self.film2(x1, feature_vec2)
        ...
        # stage 4
        feature_vec3 = feature_vec.expand_as(torch.ones(B,c,H1,W1))
        x1 = self.film3(x1, feature_vec3)
    ...
    def forward(self, x, feature_vec):
        x = self.forward_features(x, feature_vec)
...
class Mlp(nn.Module):
    def __init__(self, in_features, hyper=False, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        ...
        if hyper == False:
            ...
            self.hypernet = nn.Sequential(nn.Linear(64, hidden_features*3),
                                          nn.ReLU(),
    ...
    def forward(self, x, H, W, vec):
        ...
        if self.hyper==False:
            ...
            B, N, C = x.shape
            #print("MLP input x shape: ", x.shape)
            x = x.transpose(1, 2).view(B, C, H, W).contiguous()
            weight = self.hypernet(vec).reshape(-1, 1, 3, 3)   # B*C, 1, kH, kW  #cout, cin/groups, kH, kW
            out = F.conv2d(x.view(1, -1, H, W), weight, groups=B*C, padding=1)
...
class Attention(nn.Module):
    def __init__(self, dim, hyper=False, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        super().__init__()
        ...
        if hyper==True:
            self.hypernet1 = nn.Sequential(nn.Linear(64, 64),
                                              nn.ReLU(),
                                              nn.Linear(64, dim*dim))
            self.hypernet2 = nn.Sequential(nn.Linear(64, 64),
                                              nn.ReLU(),
    ...
    def forward(self, x, H, W, vec):
        ...
        if self.hyper==True:    #using hyper network
            Wq = self.hypernet1(vec).reshape(B, self.dim, self.dim)   # B Cout Cin
            Wkv = self.hypernet2(vec).reshape(B, 2*self.dim, self.dim)   # B Cout Cin

            #generate q, k, v
            x1 = []
            x2 = []
            for b in range(B):
                x1.append(F.linear(x[b,:,:], Wq[b,:,:]))    #x shape: B, N, C
            ...
            if self.sr_ratio > 1:
                ...
                x_ = self.norm(x_)
                for b in range(B):
                    x2.append(F.linear(x_[b,:,:], Wkv[b,:,:]))
                x2 = torch.stack(x2, dim=0)
                kv = x2.reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            else:
                for b in range(B):
                    x2.append(F.linear(x[b,:,:], Wkv[b,:,:]))
                x2 = torch.stack(x2, dim=0)
                kv = x2.reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)

        else:   #don't use hyper network
...
class Block_dec(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.mlp = Mlp(in_features=dim, hyper=False, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, H, W):
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W, None))
...
class Block(nn.Module):
    ...
    def __init__(self, hyper_attn, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(
            dim, hyper=hyper_attn,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        ...
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hyper=True, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class DecoderTransformer(nn.Module):
    ...
    def forward_features(self, x):
        x=x[3]    # only use the output of encoder stage 4
        B = x.shape[0]
        outs = []
...
class Network_top(nn.Module):
    ...
    def __init__(self, path=None, **kwargs):
        super(Network_top, self).__init__()
