...
def generalized_steps_overlapping(
    x, x_cond, seq, model, b, eta=0., corners=None, p_size=None, tile_bs=1
):
    assert corners is not None and p_size is not None
    n = x.size(0)
    device = x.device
    seq_next = [-1] + list(seq[:-1])
    x0_preds, xs = [], [x]
    ...
    for step_idx, (i, j) in enumerate(zip(reversed(seq), reversed(seq_next))):
        ...
        # 🔹 콘솔 출력
        print(f"[Diffusion step] {step_idx+1}/{total_steps}")

        # 🔹 wandb 기록
        try:
            import wandb
            wandb.log({
                "diffusion_step": step_idx+1,
        ...
        next_t = torch.full((n,), j, device=device)
        at     = compute_alpha(b, t.long())
        at_n   = compute_alpha(b, next_t.long())
        xt     = xs[-1].to(device)

        et_output = torch.zeros_like(x_cond, device=device)
        num_tiles = len(corners)

        for s in range(0, num_tiles, tile_bs):
            batch_corners = corners[s:s + tile_bs]
            xt_batch = torch.cat([crop(xt, hi, wi, p_size, p_size) for (hi, wi) in batch_corners], dim=0)
            xc_batch = torch.cat([data_transform(crop(x_cond, hi, wi, p_size, p_size))
                                  for (hi, wi) in batch_corners], dim=0)
            # half 추론
            with torch.amp.autocast("cuda", dtype=torch.float16):
                out = model(torch.cat([xc_batch, xt_batch], dim=1), t)
            for idx, (hi, wi) in enumerate(batch_corners):
                et_output[0, :, hi:hi + p_size, wi:wi + p_size] += out[idx]
            del xt_batch, xc_batch, out
        ...
        img = inverse_data_transform(x0_t.to('cpu'))[0]  # (C,H,W) 텐서
        import torchvision
        img_pil = torchvision.transforms.ToPILImage()(img)

        wandb.log({
            "step_image": wandb.Image(img_pil, caption=f"step_{step_idx+1}")
        })

        c1 = eta * ((1 - at / at_n) * (1 - at_n) / (1 - at)).sqrt()
        c2 = ((1 - at_n) - c1 ** 2).sqrt()
        xt_next = at_n.sqrt() * x0_t + c1 * torch.randn_like(x) + c2 * et
        xs.append(xt_next.to('cpu'))

        del et_output, et, x0_t, xt, at, at_n, t, next_t
        torch.cuda.empty_cache()
